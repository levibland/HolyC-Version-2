...
#include "memory.h"
#include "include/terrence_vm.h"

#if !terrence_MEMORY_DEBUG

void *terrence_calloc(terrence_vm *vm, size_t count, size_t size) {
    if (vm && ((count * size) >= terrence_vm_maxmemblock(vm))) {
        terrence_vm_seterror(vm, "Maximum memory allocation block size reached (req: %d, max: %lld).", (count * size), (int64_t)terrence_vm_maxmemblock(vm));
        return NULL;
    }
...
void *terrence_realloc(terrence_vm *vm, void *ptr, size_t new_size) {
    if (vm && (new_size >= terrence_vm_maxmemblock(vm))) {
        terrence_vm_seterror(vm, "Maximum memory re-allocation block size reached (req: %d, max: %lld).", new_size, (int64_t)terrence_vm_maxmemblock(vm));
        return NULL;
    }
